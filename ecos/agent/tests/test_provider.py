...
def test_place_optimization_terminal_result_appends_evidence_and_never_selects_baseline(
    tmp_path: Path,
) -> None:
    workspace = tmp_path / "source-workspace"
    _write_place_workspace(workspace)
    memory_path = tmp_path / "experiment-memory.jsonl"
    events: list[dict[str, object]] = []
    provider = EcosAgentProvider(
        emit=events.append,
    ...
    _send(provider, session_id, "Optimize target density")
    _send(provider, session_id, "1")
    contract = _last_event(events, "workspace_optimization")["workspaceOptimization"]
    reruns = contract["rerun_contracts"]
    result = {
        ...
        "evaluations": [
            {
                ...
                "status": "succeeded",
                "metrics": {"place_hpwl": 1 if index == 0 else 10 + index},
    ...
    _send(provider, session_id, "workspace_optimization_result:" + json.dumps(result))

    records = [json.loads(line) for line in memory_path.read_text(encoding="utf-8").splitlines()]
    assert [record["status"] for record in records] == ["planned"] * 5 + ["succeeded"] * 5
    assert all(record["fixed_rpc_operation"] == "candidate.rerun" for record in records)
    assert provider.sessions[session_id].phase == "operation"
    assert "candidate_1" in _last_event(events, "message")["text"]
    assert "not a PPA claim" in _last_event(events, "message")["text"]


def test_place_optimization_rejects_terminal_results_not_bound_to_the_frozen_contract(tmp_path: Path) -> None:
    workspace = tmp_path / "source-workspace"
    _write_place_workspace(workspace)
    provider = EcosAgentProvider(
        emit=lambda _event: None,
        experiment_memory_path=tmp_path / "experiment-memory.jsonl",
        place_assistant=_approved_place_assistant(tmp_path / "audit.jsonl"),
    )
    session_id = provider.start_session(
        {"directory": str(workspace), "mode": "workspace"}
    ...
    try:
        _send(
            provider,
            session_id,
            "workspace_optimization_result:" + json.dumps({"run_id": "other", "evaluations": []}),
        )
    except ValueError as exc:
        assert "optimization result" in str(exc).lower()
    else:
        raise AssertionError("Unbound optimization results must be rejected")

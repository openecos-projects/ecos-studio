...
def _rerun_path_proposal(workspace: Path) -> dict[str, object]:
    return {
        "schema_version": "flow-agent.gui_workspace_rerun_path_proposal.v1",
        "source_workspace": str(workspace),
        "summary": "Found the completed workspace.",
    }
...
def test_rerun_asks_design_then_uses_a_codex_workspace_recommendation(tmp_path: Path) -> None:
    ...
    events: list[dict[str, object]] = []
    recommendation_contexts: list[dict[str, object]] = []

    def recommend_workspace(context: dict[str, object]) -> dict[str, object]:
        recommendation_contexts.append(context)
        callback = context["_progress_callback"]
        assert callable(callback)
        callback("Codex is searching the authorized workspace roots.")
        return _rerun_path_proposal(workspace)

    provider = EcosAgentProvider(
        emit=events.append,
        rerun_workspace_recommender=recommend_workspace,
    )
    ...
    assert recommendation_contexts[0]["design_name"] == "gcd"
    assert recommendation_contexts[0]["filesystem_roots"] == [str(tmp_path)]
    assert any(event["type"] == "tool" for event in events)


def test_rerun_fails_closed_without_an_authorized_workspace_root() -> None:
    events: list[dict[str, object]] = []
    ...
    assert "Open an ECOS workspace" in str(events[-2]["text"])
    assert not any(event["type"] == "workspace_rerun" for event in events)


def test_rerun_path_recommendation_fails_closed_when_codex_times_out(tmp_path: Path) -> None:
    events: list[dict[str, object]] = []
    ...
    provider = EcosAgentProvider(
        emit=events.append,
        rerun_workspace_recommender=mock_codex_timeout,
    )
    session_id = provider.start_session({"directory": str(tmp_path)})["sessionId"]
    _send(provider, session_id, "2")
    _send(provider, session_id, "gcd")

    assert provider.sessions[session_id].phase == "rerun_design"
    assert events[-2]["type"] == "error"
    assert "mock timeout" in str(events[-2]["text"])
...
def test_rerun_freezes_evidence_before_requesting_gui_execution(tmp_path: Path) -> None:
    ...
    provider = EcosAgentProvider(
        emit=events.append,
        rerun_workspace_recommender=lambda _context: _rerun_path_proposal(workspace),

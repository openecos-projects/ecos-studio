...
class CodexAppServerProposalProvider(CodexThreadManagementMixin):
    ...
    def propose_v2(
        ...
        system = (
            ...
            "Prefer a knowledge claim: when a supported_action_view action covers the chosen "
            "knob and direction, cite its complete claim, binding, knob, direction, and hash; "
            "leave all claim and binding fields null only when no supplied action covers the "
            "action. A supplied supported_action_view.inactionable_matches entry is a claim the "
            "state compiler evaluated blocked or unknown, with reason codes such as "
            "anti_condition, stale_binding, or missing_observation: never cite it, and when one "
            "covers the action you would otherwise take, prefer decision continue and name that "
            "reason in rationale_summary. When a supplied supported_action_view exposes no "
            "action, prefer continue: no knowledge-supported action exists. "

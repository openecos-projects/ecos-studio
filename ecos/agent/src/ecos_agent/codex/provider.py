...
class CodexAppServerProposalProvider(CodexThreadManagementMixin):
    ...
    def propose_v2(
        ...
        system = (
            ...
            "The action object contains exactly these keys: knob_id, direction, requested_value, "
            "effective_domain_sha256, expected_effects, and the four claim fields claim_id, "
            "claim_sha256, binding_id, binding_sha256. Fill the four claim fields with the "
            "covering supported_action_view action's values exactly as supplied, and set all "
            "four to null when no supplied action covers the chosen knob and direction. Any "
            "other key, such as claim_ref or parameter_card_ref, is rejected. Strategy steps "
            "likewise contain only: step_id, knob_id, "
            ...
            "form or revise a multi-step plan (goal plus up to six steps; each step is an object with "
            "exactly the fields step_id, knob_id, direction, requested_value (the same field name and "
            "meaning as in action; omit it or null for a to-be-decided probe), intent "
            "probe/confirm/exploit, an optional condition on one objective metric expects "
            "improved/degraded/unchanged, and depends_on step ids forming a "
            ...
            "When parameter_policy.floorplan_candidate_mode is die_util, Floorplan-target candidate reruns "
            "execute in die_util mode, so a floorplan parameter card's die_size-inactive condition describes "
            "only the baseline stage mode, not candidates: with variable geometry both floorplan knobs are "
            "effective probes and the die_size baseline is not a reason to avoid them. "
            "density_weight is advanced and must never be enabled implicitly: a supplied "
            ...
            "worst corner per STA metric; full per-corner detail stays ledger-bound via outcome_sha256. "
            "Describe cross-stage evidence with non-causal correlation language: say a margin was "
            "'consumed across placement' or 'associated with detours', never that a stage or knob "
            "'caused' or 'proved' an outcome; observational data across flow stages does not establish "
            "causation. An evidence_invalid outcome with incumbent_decision evidence_limited means that "
            "candidate's artifacts were incomplete or self-contradictory: the knob effect is unknown, "
            "not negative; keep it as diagnostic evidence and never count it against a parameter. "
            "recovery_priorities ranks active feasibility blockers by deterministic severity; "
            "qor_dimension_scores expose weighted quality bottlenecks, and engineering_states with "
            "state OPPORTUNITY mark over-provisioned margins: prioritize Tier-1 feasibility blockers "
            "first, then Tier-2 dimension bottlenecks, then harvesting Tier-3 opportunities, without "
            "treating the ranking as execution authority. "
            "Infer behavior for any "
            ...
            "When a supported_action_view action covers the chosen knob and direction, fill the "
            "four claim fields with that action's claim_id, claim_sha256, binding_id, and "
            "binding_sha256 exactly as supplied; set all four to null only when no supplied "
            "action covers the action, and never invent claim or binding values. A supplied "
            "supported_action_view.inactionable_matches entry is a claim the state compiler "
            "evaluated blocked or unknown, with reason codes such as "
            "anti_condition, stale_binding, or missing_observation: never cite it. When a "
            "supplied supported_action_view is present and every candidate covering the action "
            "you would otherwise take is blocked or unknown, decision continue is mandatory: "
            "name that reason code in rationale_summary and propose nothing this turn. When a "
            "supplied supported_action_view exposes no pass action at all, decision continue "
            "is mandatory: no knowledge-supported action exists. "
            "Unclaimed probes still run and remain episode evidence, but only claim-bound probes "
            "are distilled into the reusable empirical case pool. "
            "Raw citations do not authorize an action. A claim supporting one "
            "direction does not prohibit probing another legal direction without it. "
    ...
    def propose_optimization_objective(self, natural_language_goal: str) -> dict[str, Any]:
        ...
        proposal = self._proposal(
            ...
            (
                ...
                "advanced_parameters_enabled true unless the user explicitly declines advanced parameters. "
                "Lowering unqualified area "
                ...
                "and prohibitions from permission. Advanced density_weight is enabled by default; set "
                "advanced_parameters_enabled false only when the user explicitly excludes it. "
                ...
                "so omit these redundant preserve_metrics. Never propose "
                "gui_overall_qor_score as a preserve metric: it is a derived weighted "
                "composite, and preserving a projection while optimizing another metric "
                "is circular. Prefer direct measurements over estimated power or proxy "
                "overflow metrics as preserve constraints when the user's wording allows "
                "both. Keep the user's original primary "

...
FAILURE_DETAILS = {
    ...
    "floorplan": (
        ("engine", ("floorplan ECC unavailable", "floorplan load data failed"), "If `get_eda_instance` returns no ECC module, neither floorplan runner enters `init_fp` or the native run call and returns false."),
        ...
        ("macro_step", ("macro placement step failed", "dreamplace macro placement incomplete"), "In the `macroPlacement` step, a failed DreamPlace macro-only run or a failed `tcl_save` marks the `macro placement` subflow incomplete and returns false before persistence. Manual `macro.placements` bypass DreamPlace entirely, and a design with no unplaced hard macros is skipped as success."),
        ("macro_file", ("floorplan macro location file invalid", "macro placement file errors"), "In `file` mode, an empty path, an unreadable macro-location Tcl, malformed lines, non-finite coordinates, `placed` status before its `placeInstance`, unknown or non-block instance names, or an invalid micron DBU are logged as native errors during `init_fp` database wrapping; affected macros stay unplaced and later MacroPlacer checks surface them."),
        ...
        ("native_progress", ("floorplan native error hidden", "floorplan subflow false success"), "The floorplan runners mark their subflow entries (`init floorplan`, `create tracks`, `place io pins`, `tap cell`, `PDN`, `set clock net`, `save data`) successful without inspecting iFP native returns. Native logs and output artifacts are required to determine the actual result."),
        ("geometry", ("floorplan geometry missing", "floorplan manifest missing"), "For every floorplan-phase step, shared persistence requires `geometry_snapshot_save` and an existing geometry manifest. Either failure causes `save_data` to return false."),
    ),
    "cts": (
    ...
    "route": (
        ...
        ("conditional_sta", ("route timing unavailable", "routing sta disabled"), "A completed route does not prove timing-aware routing. Timing-driven routing is self-contained in iRT and builds its own timing engine from its configuration and the shared database state; verify route features and logs before making timing claims."),
    ...
    "drc": (
        ("engine", ("drc ECC unavailable", "drc load data failed"), "Without an ECC module, the native `init_drc -> run_drc -> destroy_drc` lifecycle and rule checking do not run."),
        ...
        ("violation_map", ("drc violation map missing", "idrc output unavailable"), "The feature translation requires the native `violation_map.json` in the step data directory; an absent file or a non-list payload makes `save_drc_feature` return false and leaves the DRC count unknown even when the wrapper subflow says success."),
        ("invalid_shape", ("drc invalid violation entry", "drc malformed violation record"), "Each violation entry must carry a non-empty string `type` and a list-shaped `shape` with a usable layer name at index 4; any malformed entry aborts feature translation rather than being counted as zero violations."),
        ("feature", ("drc feature missing", "drc count unavailable"), "The DRC count comes from the translated feature record (`drc.number` plus per-type/layer distribution). If that record is absent or malformed, a missing number must not be reported as zero violations."),
    ),
    "filler": (
    ...
    "rcx": (
        ...
        ("native_progress", ("rcx native error hidden", "rcx subflow false success"), "Failed RCX initialization or extraction marks `run rcx` incomplete and returns false; the extractor is destroyed in a `finally` block whose release failure is only logged. Explicit save-data, SPEF-copy, and SPEF-fact gates remain the checked boundaries."),

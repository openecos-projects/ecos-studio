...
METRICS = {
    ...
    "cts": ("clock_path_max_buffer", "clock_path_min_buffer", "clock_wirelength", "cts_buffer_area", "cts_buffer_count", "cts_clock_tree_max_level", "cts_clock_wirelength_max", "cts_worst_optimized_skew_ns", "cts_worst_max_insertion_latency_ns", "cts_skew_target_unmet_count"),
    "legalization": (),
    "sizer": (),
...
STAGES = (
    ...
    Stage(
        ...
        "The ECC runner loads the design and invokes the iRT pipeline through `run_routing`; timing-driven routing is self-contained in iRT, which builds its own timing engine from the shared database configuration, so the runner performs no Python-side STA initialization around routing. It saves the resulting design and geometry snapshot, then runs analysis and checklist generation. The wrapper marks the routing subflow successful without inspecting the native routing return value.",
    ...
    Stage(
        ...
        "The ECC runner loads the design, runs the native iDRC lifecycle `init_drc -> run_drc -> destroy_drc` in the step data directory, saves the design, translates the native `violation_map.json` into the DRC feature record, and then runs analysis and checklist generation. The wrapper does not branch on the native DRC lifecycle results, so the feature and report artifacts are the observable completion evidence.",
...
SOURCE_PATHS = {
    ...
    "gui.step_metrics": "ecos/gui/apps/desktop-electron/electron/services/workspaceQorAnalysis.ts",
...
PARAMETER_DETAILS = {
    "floorplan": {
        ...
        "macro_placer.mode": ("The macro-placement input mode (`auto` or `file`).", "The pre-floorplan runner forces `auto` with an empty file path; the post-floorplan runner forces `file` with the workspace macro-location Tcl, making the macro-placement step's result the fixed macro input."),
        "macro_placer.file_path": ("The macro-placement location file consumed in `file` mode.", "`postFloorplan` points it at the workspace macro-location Tcl written by the macro-placement step; `init_fp` applies its placement commands while wrapping the database."),
        ...
        "io_placer.io_layer_list": ("The routing layers eligible for IO-pin placement.", "It constrains where the floorplan can place IO pins and is required in both automatic and file-driven IO placement."),
        "io_placer.mode": ("The IO-pin placement mode (`auto` or `file`).", "`run_simple_fp` always auto-places on the configured layers; the full `run_fp` path reads `io_placer.file_path` in `file` mode and otherwise auto-places."),
        "io_placer.file_path": ("The IO-pin location file consumed in `file` mode.", "When the mode is `file`, IOPlacer reads pin locations from this file instead of running automatic edge placement; the configured layer list still resolves the pin shapes."),
    ...
    "route": {
        ...
        "RT.-enable_timing": ("The routing timing-awareness switch.", "It configures timing-driven routing inside iRT, which builds its own timing engine from the shared database configuration; the runner performs no separate STA initialization around routing."),

...
PLACE_SOURCE_PATHS = {
    ...
    "dreamplace.parameter_runtime_observer": (
        "ecc/agent/data/parameter_runtime_observer.py"
    ),
    ...
    "ecc.congestion": "ecc/chipcompiler/thirdparty/ecc-tools/src/feature/evaluation/src/module/congestion/congestion_eval.cpp",
    ...
    "ecc.density": "ecc/chipcompiler/thirdparty/ecc-tools/src/feature/evaluation/src/module/density/density_eval.cpp",
    "ecc.wirelength": "ecc/chipcompiler/thirdparty/ecc-tools/src/feature/evaluation/src/module/wirelength/wirelength_eval.cpp",
...
PLACE_PARAMETER_SEMANTICS = {
    ...
    "density_weight": ("The initial density-penalty coefficient.", "The actual value is the coefficient consumed by density-weight initialization, not the internal tensor that DREAMPlace updates during optimization or reinitializes after routability-driven area adjustment."),
    "random_seed": ("The random seed.", "It initializes Python, Torch, and CUDA random state for reproducible initial perturbations. Under a fixed seed the placement chain is bitwise-deterministic, producing identical signoff QoR across replays, so a seed change is a real QoR variable rather than measurement noise; ECOS pins candidate reruns to the workspace baseline seed so cross-candidate comparisons stay seed-matched."),
    ...
    "cell_padding_x": ("The standard-cell padding along X, in sites.", "DREAMPlace quantizes and may cap the padding before expanding movable-cell and pin geometry. A requested zero is effective when confirmed as zero; a positive request clipped to zero is inactive. The actual value remains the placement padding in sites, not the geometry restored for legalization."),
...
def _add_parameters(entries: list[dict[str, object]], documents: dict[str, list[str]], config: dict[str, object]) -> None:
    for name, default in config.items():
        ...
        if consumer_sources:
            evidence += ("dreamplace.parameter_runtime_observer", *consumer_sources)

...
from ecos_agent.optimization.parameters.acceptance_artifacts import (
    build_entry_payload,
    candidate_evidence,
    candidate_paths,
...
def _artifact_issues(candidate_root: Path, payload: dict, paths: dict[str, Path]) -> list[str]:
    issues: list[str] = []
    ...
    for key, path_key in (
        ...
        if not path.is_file():
            ...
            continue
        expected_ref = path.relative_to(candidate_root).as_posix()
        if declared_artifact.get("ref") != expected_ref:
            issues.append(f"{key} ref mismatch")
        if declared_artifact.get("sha256") != file_sha256(path):
            # Keep processing so the report contains all independent failures.
            issues.append(f"{key} hash mismatch")
    return issues


def _validate_materialization(materialization_payload: dict, issues: list[str]) -> None:
...
def _validate_replay(
    ...
    for key, message in (
        ("candidate_id", "replay candidate id mismatch"),
        ("candidate_root_ref", "replay candidate root mismatch"),
        ("parent_candidate_root_ref", "replay parent mismatch"),
        ("target_step", "replay target step mismatch"),
        ("end_step", "replay end step mismatch"),
        ("execution_scope", "replay execution scope mismatch"),
        ("parent_flow_sha256", "replay parent flow mismatch"),
    ):
        if replay_payload.get(key) != payload.get(key):
    ...
    if replay_payload.get("candidate_manifest_sha256") != file_sha256(paths["manifest"]):
...
def _match_trace(
    ...
    evidence: CandidateExecutionEvidence,
    native_receipt: ParameterApplicationReceipt | None,
    terminal_sha256: str,
    card_sha256: str,
    issues: list[str],
) -> Path | None:
...
def _load_candidate_receipts(
    ...
    *,
    ...
    expected_ecc_revision: str,
    issues: list[str],
) -> tuple[dict, ParameterApplicationReceipt | None]:
    receipt = _read_json_artifact(paths["receipt"], "native receipt", issues)
    runtime = _read_json_artifact(paths["runtime_report"], "runtime report", issues)
    materialization = _read_json_artifact(
        paths["materialization"], "materialization receipt", issues
    )
    _validate_materialization(materialization, issues)
    native, receipt_issues = _validate_native_receipt(
        receipt,
        runtime,
        materialization,
    ...
    if receipt.get("context", {}).get("ecc_revision") != expected_ecc_revision:
        issues.append("candidate ECC revision does not match expected revision")
    if native is None or not _receipt_is_effective(receipt):
...
def _candidate_entry(
    ...
    *,
    ...
    paths = candidate_paths(workspace, candidate_id)
    payload = json.loads(paths["manifest"].read_text(encoding="utf-8"))
    issues = _artifact_issues(paths["root"], payload, paths)
    evidence = candidate_evidence(payload, paths["manifest"])
    observation = build_candidate_terminal_observation(workspace, evidence)
    observation_path = write_terminal_observation(output, knob_id, observation)
    receipt_payload, native_receipt = _load_candidate_receipts(
        workspace,
        paths,
        payload,
        evidence,
    ...
    trace_episode = _match_trace(
        episode_replays, evidence, native_receipt, terminal_sha256, card_sha256, issues
    )
    return build_entry_payload(
        ...
        replay_available=replay_payload is not None
        and not any(
...
def build_acceptance(
    ...
    *,
    ...
    return write_acceptance_outputs(
        output, workspace, entries, provenance, IGNORED_KNOBS
    )

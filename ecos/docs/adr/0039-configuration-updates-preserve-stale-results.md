...
During a partial, failed or cancelled rerun, each successfully committed Step uses its current-revision result, the failed or cancelled Step shows its current state, and untouched invalidated Steps may show predecessor facts only as labeled read-only evidence. Current QoR, Project Comparison, Signoff and execution dependencies never substitute predecessor data for missing current-revision results.

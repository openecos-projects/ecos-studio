...
function canonicalConfiguration(config: ConfigData) {
  return {
    ...
    parameters: {
      frequency_max: config.frequencyMax,
      core_utilization: config.core.utilization,
      die_width: config.die.Size[0] ?? 0,
      die_height: config.die.Size[1] ?? 0,
      core_margin: [...config.core.margin],
      aspect_ratio: config.core.aspectRatio,
      max_fanout: config.maxFanout,
      target_density: config.targetDensity,
      target_overflow: config.targetOverflow,
      global_right_padding: config.globalRightPadding,
      cell_padding_x: config.cellPaddingX,
      routability_opt_flag: config.routabilityOptFlag ? 1 : 0,
      bottom_layer: FIXED_BOTTOM_LAYER,
      top_layer: FIXED_TOP_LAYER,
    },
  }
}

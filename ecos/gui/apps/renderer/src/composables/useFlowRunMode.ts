...
export function useFlowRunMode(
  currentStage: Ref<string | undefined | null>,
  options: { fullFlowLabel?: Ref<string> } = {},
) {
  const fullFlowRunMode = ref<FlowRunModeKey>('run')
  const stepRunMode = ref<FlowRunModeKey>('run')
  const isFullFlowContext = computed(() => currentStage.value === 'home')

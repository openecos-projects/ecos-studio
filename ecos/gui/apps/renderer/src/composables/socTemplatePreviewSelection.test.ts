...
const template = {
  ...
  cores: [
    ...
    },
    {
      ...
      orient: 'N',
      selected: 0,
      boundingBox: { llx: 10, lly: 10, urx: 30, ury: 30, width: 20, height: 20 },
    },
    {
      id: 9,
      name: 'core9',
      ...
      orient: 'FN',
      selected: 0,
      boundingBox: { llx: 40, lly: 40, urx: 60, ury: 60, width: 20, height: 20 },
    },
  ],
  ioPins: [],
...
describe('socTemplatePreviewSelection', () => {
  ...
  it('uses the core marked selected in JSON before falling back to the first valid core', () => {
    expect(
      getDefaultSocCoreId({
        ...template,
        cores: template.cores.map((core) => ({
          ...core,
          selected: core.id === 9 ? 1 : 0,
        })),
      }),
    ).toBe(9)
  })

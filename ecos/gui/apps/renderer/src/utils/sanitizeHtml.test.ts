...
describe('sanitizeHtml helpers', () => {
  ...
  it('keeps safe URLs and image data URLs', () => {
    expect(sanitizeUrl('https://example.com/report', 'href')).toBe(
      'https://example.com/report',
    )
    expect(sanitizeUrl('/tmp/report.html', 'href')).toBe('/tmp/report.html')
    expect(sanitizeUrl('data:image/png;base64,AAAA', 'src')).toBe(
      'data:image/png;base64,AAAA',
    )
  })

  it('drops suspicious class tokens', () => {
    expect(sanitizeClassList('ok danger<script bad_token good:one')).toBe(
      'ok bad_token good:one',
    )
  })

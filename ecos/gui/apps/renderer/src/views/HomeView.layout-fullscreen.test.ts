...
describe('HomeView workspace dashboard layout', () => {
  ...
  it('uses grid cells for dashboard parameters and compact QoR comparison entry points', () => {
    ...
    expect(homeViewSource).not.toContain("state.baselineSource === 'default'")
    expect(homeViewSource).not.toContain('Default baseline')
    ...
    expect(homeViewSource).toContain('PASS >= ${threshold}')
    expect(homeViewSource).toContain('FAIL < ${threshold}')
    expect(homeViewSource).toContain('state.comparison?.baselineScore')
    expect(homeViewSource).toContain('comparison?.scoreThreshold')
    expect(homeViewSource).not.toContain('@/utils/projectQorTrend')
  ...
  it('shows the requested chip metadata and max fanout from Home parameters', () => {
    ...
    expect(chipCard).toContain('valueOrNA(workspaceIdentity?.projectName)')
    ...
    expect(homeViewSource).toContain('useBackendWorkspaceSession()')

...
describe('DrawingArea native layout viewer bridge', () => {
  it('wires the current view JSON package root to the desktop native viewer API', () => {
  ...
  it('shows a canvas transition while the native viewer package is prepared', () => {
    ...
    expect(source).toContain('layoutState.loadingMessage.value = NATIVE_LAYOUT_VIEWER_LOADING_MESSAGE')

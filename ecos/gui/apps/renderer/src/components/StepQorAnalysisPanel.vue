<template>
  <div class="step-qor-root flex h-full min-h-0 w-full min-w-0 flex-col bg-(--bg-primary)">
    <header class="flex shrink-0 items-start justify-between border-b border-(--border-color) px-3 py-2">
      <div class="min-w-0">
        <h2 class="truncate text-[12px] font-bold text-(--text-primary)">{{ panelTitle }}</h2>
    ...
    <div v-if="loading" class="flex min-h-0 flex-1 flex-col items-center justify-center px-4 text-center">
    ...
    <div v-else-if="error" class="m-3 rounded border border-red-500/40 bg-red-500/10 p-3">
      ...
      <button type="button" class="mt-3 text-[11px] text-(--accent-color) hover:underline" @click="refetch">
        Retry
      </button>
    </div>
    ...
    >
      <i class="ri-bar-chart-box-line mb-3 text-4xl text-(--text-secondary) opacity-40"></i>
    ...
    >
      ...
      <p v-if="messages.length" class="mt-2 text-[10px] leading-relaxed text-(--text-secondary)">
        {{ messages.join(' ') }}
      </p>
    </div>
    ...
    <div v-else class="min-h-0 flex-1 overflow-auto">
      <section v-if="kind === 'route'" class="min-w-max">
        <div class="flex items-center justify-between border-b border-(--border-color) px-3 py-2 text-[10px] text-(--text-secondary)">
          ...
          <span v-if="routeFinalIteration !== null">Final DR iteration {{ routeFinalIteration }}</span>
        </div>
        <table class="w-full border-collapse text-left text-[11px] tabular-nums" aria-label="Route layer QoR analysis">
          <thead class="sticky top-0 bg-(--bg-secondary) text-[10px] text-(--text-secondary)">
            <tr>
              <th class="sticky left-0 z-10 bg-(--bg-secondary) px-3 py-2 font-semibold">Layer</th>
          ...
          <tbody>
            <tr v-for="layer in routeLayers" :key="layer.id" class="border-b border-(--border-color)/70 hover:bg-(--bg-hover)">
              <th class="sticky left-0 bg-(--bg-primary) px-3 py-2 font-medium text-(--text-primary)">{{ layer.label }}</th>
              <td class="px-2 py-2">{{ formatMetric(layer.la.demand) }}</td>
              <td class="px-2 py-2" :class="numberRiskClass(layer.la.overflow)">{{ formatMetric(layer.la.overflow) }}</td>
              ...
              <td class="px-2 py-2" :class="numberRiskClass(layer.dr.violationCount)">{{ formatMetric(layer.dr.violationCount) }}</td>
      ...
      <section v-else-if="kind === 'place'" class="min-w-max">
        <div class="border-b border-(--border-color) px-3 py-2 text-[10px] text-(--text-secondary)">
          {{ placeMaps.length }} declared map sources
        </div>
        <table class="w-full border-collapse text-left text-[11px] tabular-nums" aria-label="Place map QoR analysis">
          <thead class="sticky top-0 bg-(--bg-secondary) text-[10px] text-(--text-secondary)">
            <tr>
          ...
          <tbody>
            <tr v-for="map in placeMaps" :key="map.id" class="border-b border-(--border-color)/70 hover:bg-(--bg-hover)" :title="map.sourceFile">
              <td class="px-3 py-2 font-medium text-(--text-primary)">{{ map.group }}</td>
              <td class="px-2 py-2">{{ map.metric }}</td>
              <td class="px-2 py-2 text-(--text-secondary)">{{ map.direction || '-' }}</td>
              <td class="px-2 py-2">{{ formatMetric(map.max) }}</td>
              <td class="px-2 py-2">{{ formatMetric(map.topAverage) }}</td>
              <td class="px-2 py-2">{{ formatRatio(map.nonzeroCount, map.valueCount) }}</td>
              <td class="px-3 py-2" :class="numberRiskClass(map.highBinCount)">{{ formatRatio(map.highBinCount, map.valueCount) }}</td>
            </tr>
          </tbody>
        </table>
      </section>
      ...
      <section v-else-if="kind === 'sta'" class="min-w-max">
        <div class="flex border-b border-(--border-color) px-3 pt-2" role="tablist" aria-label="STA QoR analysis view">
          <button type="button" role="tab" :aria-selected="staView === 'groups'" :class="staTabClass('groups')" @click="staView = 'groups'">Path groups</button>
          <button type="button" role="tab" :aria-selected="staView === 'corners'" :class="staTabClass('corners')" @click="staView = 'corners'">Corner records</button>
        </div>
        <table v-if="staView === 'groups'" class="w-full border-collapse text-left text-[11px] tabular-nums" aria-label="STA path group summary">
          <thead class="sticky top-0 bg-(--bg-secondary) text-[10px] text-(--text-secondary)">
            <tr>
          ...
          <tbody>
            <tr v-for="group in staGroups" :key="group.name" class="border-b border-(--border-color)/70 hover:bg-(--bg-hover)">
              <th class="px-3 py-2 font-medium text-(--text-primary)" :title="group.cornerHint">{{ group.name }}</th>
              <td class="px-2 py-2" :class="timingRiskClass(group.setup.wns)">{{ formatMetric(group.setup.wns) }}</td>
              <td class="px-2 py-2" :class="timingRiskClass(group.setup.tns)">{{ formatMetric(group.setup.tns) }}</td>
              <td class="px-2 py-2">{{ formatMetric(group.setup.frequency) }}</td>
              <td class="px-2 py-2" :class="numberRiskClass(group.setup.nvp)">{{ formatMetric(group.setup.nvp) }}</td>
              <td class="px-2 py-2" :class="timingRiskClass(group.hold.wns)">{{ formatMetric(group.hold.wns) }}</td>
              <td class="px-2 py-2" :class="timingRiskClass(group.hold.tns)">{{ formatMetric(group.hold.tns) }}</td>
              <td class="px-3 py-2" :class="numberRiskClass(group.hold.nvp)">{{ formatMetric(group.hold.nvp) }}</td>
            </tr>
          </tbody>
        </table>
        <table v-else class="w-full border-collapse text-left text-[11px] tabular-nums" aria-label="STA corner path group records">
          <thead class="sticky top-0 bg-(--bg-secondary) text-[10px] text-(--text-secondary)">
            <tr>
          ...
          <tbody>
            <tr v-for="record in staRecords" :key="record.id" class="border-b border-(--border-color)/70 hover:bg-(--bg-hover)" :title="record.sourceFile">
              <th class="px-3 py-2 font-medium text-(--text-primary)">{{ record.corner }}</th>
              <td class="px-2 py-2">{{ record.pathGroup }}</td>
              <td class="px-2 py-2" :class="timingRiskClass(record.setup.wns)">{{ formatMetric(record.setup.wns) }}</td>
              <td class="px-2 py-2" :class="timingRiskClass(record.setup.tns)">{{ formatMetric(record.setup.tns) }}</td>
              <td class="px-2 py-2" :class="numberRiskClass(record.setup.nvp)">{{ formatMetric(record.setup.nvp) }}</td>
              <td class="px-2 py-2">{{ formatMetric(record.setup.frequency) }}</td>
              <td class="px-2 py-2" :class="timingRiskClass(record.hold.wns)">{{ formatMetric(record.hold.wns) }}</td>
              <td class="px-2 py-2" :class="timingRiskClass(record.hold.tns)">{{ formatMetric(record.hold.tns) }}</td>
              <td class="px-3 py-2" :class="numberRiskClass(record.hold.nvp)">{{ formatMetric(record.hold.nvp) }}</td>
            </tr>
          </tbody>
        </table>
      </section>

      <footer v-if="metricsPath" class="border-t border-(--border-color) px-3 py-2 text-[10px] text-(--text-secondary)" :title="metricsPath">
...
const routeLayers = computed(() => {
  const layers = isRecord(detail.value) && Array.isArray(detail.value.layers) ? detail.value.layers : []
...
const placeMaps = computed(() => {
  const maps = isRecord(detail.value) && Array.isArray(detail.value.maps) ? detail.value.maps : []
...
const staGroups = computed(() => {
  const groups = isRecord(detail.value) && Array.isArray(detail.value.path_groups)
  ...
  return groups.filter(isRecord).map((group, index) => {
    ...
    return {
      ...
      cornerHint: [stringField(setup, 'worst_wns_corner'), stringField(hold, 'worst_wns_corner')]
        .filter(Boolean)
        .join(' / '),
      setup: {
...
const staRecords = computed(() => {
  const records = isRecord(detail.value) && Array.isArray(detail.value.records)

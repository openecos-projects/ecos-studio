...
describe('StepLogEventBridge', () => {
  ...
  it('holds a terminal event behind the archive when no completion exists', () => {
    vi.useFakeTimers()
    const h = harness({ holdTimeoutMs: 2000 })
    writeFlowJson(h.workspace, [{ name: 'Synthesis', tool: 'yosys' }])
    startOperation(h)
    startStep(h, 'Synthesis', 'yosys')
    // The executor raises after begin: no end marker, no step.completed.
    h.feed(v1Marker('begin', 'Synthesis', 'yosys'), Buffer.from('crashing output\n'))

    h.bridge.handleProtocolEvent(
      protocolEvent('operation.failed', { error: { message: 'x' } }, { sequence: 3 }),
      h.forward,
    )
    // The terminal event is held while the archive is active.
    expect(h.forwarded.map((event) => event.type)).toEqual([
      'operation.started',
      'step.started',
    ])
    ...
    // The next operation's begin starts a fresh attempt, not a nested one.
    h.bridge.handleProtocolEvent(
      protocolEvent(
        'operation.started',
        {},
    ...
    h.feed(v1Marker('begin', 'Synthesis', 'yosys'), Buffer.from('fresh attempt\n'))
    h.feed(v1Marker('end', 'Synthesis', 'yosys'))

    expect(h.archiveText('Synthesis', 'yosys')).toBe('fresh attempt\n')
    // The crashed attempt's segment was synthesized under operation-1 before
    // the terminal; nothing from it is re-attributed to operation-2.
    const synthesized = h.emitted.filter((event) => event.type === 'step.log')
    expect(synthesized.map((event) => [event.operationId, event.payload.chunk])).toEqual([
      ['operation-1', 'crashing output\n'],
      ['operation-2', 'fresh attempt\n'],
    ])
    expect(h.unscoped.join('')).not.toContain('ECC-STEP')
  })

  it('releases a held terminal immediately when the end marker arrives', () => {
    vi.useFakeTimers()
    const h = harness({ holdTimeoutMs: 2000 })
    writeFlowJson(h.workspace, [{ name: 'Synthesis', tool: 'yosys' }])
    startOperation(h)
    startStep(h, 'Synthesis', 'yosys')
    h.feed(v1Marker('begin', 'Synthesis', 'yosys'), Buffer.from('slow finish\n'))
    ...
    // The end marker arrives before the timeout: the terminal releases
    // without the abandonment note, and the timer is cleared.
    h.feed(v1Marker('end', 'Synthesis', 'yosys'))
    expect(h.forwarded.map((event) => event.type)).toEqual([
      'operation.started',
      'step.started',

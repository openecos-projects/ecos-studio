...
export interface ShimScriptOptions {
  isPackaged: boolean
  /** Bundle home root (packaged) or shared env dir (development, external). */
  dataDir: string
  /** Repository runtime-bin shim (development mode only). */
  devWrapper: string
  /** External ECC bin directory used read-only (wins over both modes). */
  externalBinDir?: string | null
}

export function buildShimScript(options: ShimScriptOptions): string {
  if (options.externalBinDir) {
    const envPath = join(options.dataDir, 'env')
    const executable = join(options.externalBinDir, 'ecc')
    return [
      '#!/bin/sh',
      '# Generated by ECOS Studio (external ECC runtime, used read-only).',

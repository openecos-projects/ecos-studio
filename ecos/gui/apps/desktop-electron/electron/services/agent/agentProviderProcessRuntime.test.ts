...
describe('AgentProviderProcessRuntime', () => {
  ...
  it('forwards only a validated frozen place optimization contract', () => {
    const harness = createSpawnHarness()
    const runtime = new AgentProviderProcessRuntime({
      manifest: {
        command: 'local-provider',
    ...
    const listener = vi.fn()
    runtime.onEvent(listener)
    const reruns = Array.from({ length: 5 }, (_, index) => ({
      design_id: 'gcd',
      end_step: 'place',
      execution_scope: 'single_step',
      parameter_patch:
        index === 0 ? [] : [{ knob_id: 'place.target_density', value: 0.8 - index / 10 }],
      requires_gui_review: true,
      rerun_id: `scan_baseline_${index}`,
      schema_version: 'flow-agent.workspace_rerun_contract.v1',
      source_stage_artifact: 'place_dreamplace/output/gcd_place.def.gz',
      source_flow_json_sha256: `sha256:${'a'.repeat(64)}`,
      source_stage_artifact_sha256: `sha256:${'b'.repeat(64)}`,
      source_workspace: '/runs/gcd',
      target_step: 'place',
      target_workspace: `/runs/gcd_scan_${index}`,
    }))
        ...
        event: {
          ...
          workspaceOptimization: {
            ...
            request: {
              ...
              strategy_id: 'strategy_place_reduce_density',
    ...
    expect(listener).toHaveBeenCalledWith(
      expect.objectContaining({
        type: 'workspace_optimization',
        workspaceOptimization: expect.objectContaining({
          run_spec: expect.objectContaining({ budget: 5, run_id: 'scan_001' }),
        }),
      }),
    )
  })

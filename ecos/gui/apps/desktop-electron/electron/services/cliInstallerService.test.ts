...
/** A minimal external ECC layout: executable `ecc` beside `_internal`. */
function createExternalEccBinDir(root: string): string {
  const binDir = join(root, 'external-ecc')
  mkdirSync(join(binDir, '_internal'), { recursive: true })
  const eccPath = join(binDir, 'ecc')
  writeFileSync(eccPath, '#!/bin/sh\necho ecc-external\n')
  chmodSync(eccPath, 0o755)
  return binDir
}
...
describe('CliInstallerService', () => {
  ...
  it('reports an external runtime as ready without contacting the registry', async () => {
    const root = createTempDir('ecos-cli-installer-external-')
    const externalBin = createExternalEccBinDir(root)
    const { service, resourceManager } = createService({
    ...
    expect(status.versionDir).toBe(externalBin)
    expect(status.installedVersion).toBe('1.0.0')
    expect(status.warning).toBeNull()
    expect(resourceManager.downloadRegistryAssetToDirectory).not.toHaveBeenCalled()
    expect(resourceManager.resolveRegistryToolAsset).not.toHaveBeenCalled()
  })

  it('warns when the external runtime version drifts from the pinned version', async () => {
    const root = createTempDir('ecos-cli-installer-external-drift-')
    const externalBin = createExternalEccBinDir(root)
    const { service } = createService({
      externalBinDir: externalBin,
      expectedVersion: '1.0.0',
      spawnResult: { code: 0, output: 'ecc 2.0.0' },
    })

    const status = await service.status()
    expect(status.status).toBe('ready')
    expect(status.installedVersion).toBe('2.0.0')
    expect(status.warning).toContain('2.0.0')
    expect(status.warning).toContain('1.0.0')
    expect(status.error).toBeNull()
  })

  it('reports an external runtime self-check failure', async () => {
    const root = createTempDir('ecos-cli-installer-external-broken-')
    const externalBin = createExternalEccBinDir(root)
    const { service } = createService({
      externalBinDir: externalBin,
      spawnResult: { code: 1, output: 'loader error' },
    })

    const status = await service.status()
    expect(status.status).toBe('self-check-failed')
    expect(status.source).toBe('external')
    expect(status.error).toContain('loader error')
  })

  it('configures an external runtime without acquisition and points the shim at it', async () => {
    const root = createTempDir('ecos-cli-installer-external-ensure-')
    const externalBin = createExternalEccBinDir(root)
    const { service, dataDir, binDir, resourceManager } = createService({
      externalBinDir: externalBin,
      spawnResult: { code: 0, output: 'ecc 1.0.0' },
    })

    const versionDir = await service.ensureBundle({ installShim: true })
    expect(versionDir).toBe(externalBin)
    expect(resourceManager.downloadRegistryAssetToDirectory).not.toHaveBeenCalled()

    const envContent = readFileSync(join(dataDir, 'env'), 'utf8')
    expect(envContent).toContain(externalBin)
    const shimContent = readFileSync(join(binDir, 'ecos-ecc'), 'utf8')
    expect(shimContent).toContain('# Generated by ECOS Studio')
    expect(shimContent).toContain(`exec '${join(externalBin, 'ecc')}' "$@"`)
    expect(shimContent).not.toContain('current')
    // Nothing was written into the external tree.
    expect(existsSync(join(externalBin, 'env'))).toBe(false)
  })

  it('uninstalls only the shim when an external runtime is active', async () => {
    const root = createTempDir('ecos-cli-installer-external-uninstall-')
    const bundle = createFakeEccBundle(root)
    // A managed install exists from before the override was configured.
    const managed = createService({ resourcesPath: bundle.resourcesPath })
    await managed.service.ensureBundle({ installShim: true })
    expect(existsSync(join(managed.dataDir, 'current'))).toBe(true)
    ...
    await externalService.uninstall()
    expect(existsSync(join(managed.binDir, 'ecos-ecc'))).toBe(false)
    // The managed bundle and the external tree both survive.
    expect(existsSync(join(managed.dataDir, 'current'))).toBe(true)
    expect(existsSync(join(externalBin, 'ecc'))).toBe(true)
  })

  it('skips startup drift maintenance when an external runtime is active', async () => {
    const root = createTempDir('ecos-cli-installer-external-startup-')
    const externalBin = createExternalEccBinDir(root)
    const { service, dataDir, resourceManager } = createService({
      externalBinDir: externalBin,
    })
    // A dangling managed link would normally trigger a repair reinstall.
    mkdirSync(dataDir, { recursive: true })
    symlinkSync('missing-target', join(dataDir, 'current'))
  ...
  it('reports the external runtime instead of the dev wrapper in development mode', async () => {
    const root = createTempDir('ecos-cli-installer-external-dev-')
    const externalBin = createExternalEccBinDir(root)
    const service = new CliInstallerService({
      resourceManager: createResourceManagerDouble(),
      env: { PATH: '/usr/bin' },

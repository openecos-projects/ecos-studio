...
describe('RemoteContentService', () => {
  ...
  it('lists files from a built-in GitHub source under its root path', async () => {
    const fetchImpl = vi.fn(async (...args: Parameters<typeof fetch>) => {
      expect(String(args[0])).toBe(
        'https://api.github.com/repos/openecos-projects/ecos-studio/git/trees/main?recursive=1',
      )
      return jsonResponse({
        truncated: false,
        tree: [
          {
            path: 'ecos/gui/apps/renderer/public/ysyxSoCASIC.json',
            type: 'blob',
            size: 123,
            sha: 'abc',
          },
          ...
          {
            path: 'ecos/gui/apps/renderer/public/image.png',
            type: 'blob',
            size: 20,
            sha: 'ghi',
          },
    ...
    await expect(
      service.listFiles({ source: 'socTemplateCatalog', pattern: '**/*.json' }),
    ).resolves.toEqual([
      {
  ...
  it('reads a text file through the GitHub contents raw media type', async () => {
    const fetchImpl = vi.fn(async (...args: Parameters<typeof fetch>) => {
      ...
      expect(String(url)).toBe(
        'https://api.github.com/repos/openecos-projects/ecos-studio/contents/ecos%2Fgui%2Fapps%2Frenderer%2Fpublic%2FysyxSoCASIC.json?ref=main',
      )
    ...
    await expect(
      service.readTextFile({ source: 'socTemplateCatalog', path: 'ysyxSoCASIC.json' }),
  ...
  it('reads files from repository root sources without prefixing an empty path segment', async () => {
    const fetchImpl = vi.fn(async (...args: Parameters<typeof fetch>) => {
      const [url] = args
      expect(String(url)).toBe(
        'https://api.github.com/repos/KoEkko/ecos-registry/contents/manifest.json?ref=main',
      )
    ...
    await expect(
      service.readTextFile({ source: 'socTemplateCatalog', path: 'manifest.json' }),
  ...
  it('lists files from repository root sources', async () => {
    const fetchImpl = vi.fn(async (...args: Parameters<typeof fetch>) => {
      expect(String(args[0])).toBe(
        'https://api.github.com/repos/KoEkko/ecos-registry/git/trees/main?recursive=1',
      )
      return jsonResponse({
        truncated: false,
        tree: [
  ...
  it('stores successful SoC catalog reads in the XDG cache directory', async () => {
    ...
    await expect(
      readFile(
        join(xdgCacheHome, 'ecos-studio', 'soc-templates', 'manifest.json'),
        'utf8',
      ),
    ).resolves.toBe('{"schema_version":1}')
  })
  ...
  it('uses ECOS_STUDIO_SOC_TEMPLATE_CACHE_DIR before XDG_CACHE_HOME for SoC catalog cache files', async () => {
    ...
    await expect(readFile(join(overrideCacheDir, 'manifest.json'), 'utf8')).resolves.toBe(
      '{"schema_version":2}',
    )
    await expect(
      readFile(
  ...
  it('falls back to a parseable SoC catalog cache file when the remote read fails', async () => {
    ...
    await expect(
      service.readJsonFile({ source: 'socTemplateCatalog', path: 'manifest.json' }),
    ).resolves.toEqual({ schema_version: 3 })
  })
  ...
  it('returns fresh SoC catalog content when only the cache write fails', async () => {
    ...
    const service = new RemoteContentService({
      fetchImpl: vi.fn(
        async () => new Response('{"schema_version":"fresh"}', { status: 200 }),
      ),
      sources: {
        socTemplateCatalog: {
          provider: 'github',
    ...
    await expect(
      service.readTextFile({ source: 'socTemplateCatalog', path: 'manifest.json' }),
    ).resolves.toBe('{"schema_version":"fresh"}')
  })
  ...
  it('does not fall back to an unparseable SoC catalog cache file for JSON reads', async () => {
    ...
    await expect(
      service.readJsonFile({ source: 'socTemplateCatalog', path: 'manifest.json' }),
    ).rejects.toThrow('Remote JSON is invalid: socTemplateCatalog/manifest.json')
  })
  ...
  it('parses JSON files and reports invalid JSON with the source path', async () => {
    ...
    await expect(
      service.readJsonFile({ source: 'socTemplateCatalog', path: 'broken.json' }),
    ).rejects.toThrow('Remote JSON is invalid: socTemplateCatalog/broken.json')
  })
  ...
  it('rejects path traversal outside the configured source root', async () => {
    ...
    await expect(
      service.readTextFile({ source: 'socTemplateCatalog', path: '../secret.json' }),
    ).rejects.toThrow('Remote content path must be relative to its source root.')
  })
  ...
  it('fails clearly when GitHub tree results are truncated', async () => {
    ...
    await expect(service.listFiles({ source: 'socTemplateCatalog' })).rejects.toThrow(
      'GitHub tree response for socTemplateCatalog is truncated.',
    )
  })
  ...
  it('reports non-OK GitHub responses with status and request URL', async () => {
    ...
    await expect(
      service.readTextFile({ source: 'socTemplateCatalog', path: 'missing.json' }),
    ).rejects.toThrow(

...
describe('registerIpc', () => {
  ...
  it('binds optimization tokens to the owner window and frozen candidate order', async () => {
    let emitAgentEvent: ((event: Record<string, unknown>) => void) | undefined
    const agentRuntimeService = {
      interrupt: vi.fn(async () => {}),
      onEvent: vi.fn((listener) => {
    ...
    const session = { providerId: 'ecos_agent', sessionId: 'optimization-session' }
    const rerun = (id: string, density?: number) => ({
      design_id: 'gcd',
      ...
      source_stage_artifact_sha256: 'b'.repeat(64),
      source_workspace: '/runs/gcd',
      target_step: 'place' as const,
      target_workspace: `/runs/${id}`,
    })
    const contracts = [
      rerun('scan_baseline'),
      rerun('scan_candidate_1', 0.7),
      rerun('scan_candidate_2', 0.6),
      rerun('scan_candidate_3', 0.5),
      rerun('scan_candidate_4', 0.4),
    ]
    fromWebContents.mockReturnValue(window)
